...
export function Logo({ className = '', onClick, size = 'sm' }: LogoProps) {
  return (
    <Link
      ...
      className={`group/logo inline-flex items-center gap-2.5 font-display font-semibold uppercase text-fg focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-signal ${
    ...
    >
      ...
      <span>
        Видеопродакшн<span className="text-signal-text transition-colors duration-300 group-hover/logo:text-fg">.рф</span>
      </span>
    </Link>
  )
}
